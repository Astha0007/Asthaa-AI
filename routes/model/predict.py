...
def load_model():
    ...
    if MODEL_PATH.exists():
        try:
            ...
            # Try loading with custom objects to handle BatchNormalization issues
            custom_objects = {
            ...
            try:
                ...
                logger.warning(f"Standard loading failed: {e1}, trying alternative method...")
                try:
                    ...
                    logger.warning(f"Alternative loading failed: {e2}, trying safe mode...")
                    # Last resort: try loading with safe_mode
                    _model = tf.keras.models.load_model(str(MODEL_PATH), safe_mode=False)

            logger.info("✅ Model loaded successfully.")
            return _model

        except Exception as e:
            logger.error(f"Failed to load model after all attempts: {e}")
            return None
    else:
        logger.warning(
